...
- [ ] **Lap-dance rooms out the back of HAPPY HOGS and BILLY JEANS** (the franchise frontages from TASK-084 are exterior-only today; the flagship venues live in `tusouxroeNorth.js`, `buildVenue`): the player hires a dancer — a hog-themed dancer at Happy Hogs, an original, fictional **80s-style dancer** at Billy Jeans (neon, sequins, synth-pop era; an invented adult character, not modelled on any real person) — for a private dance, with the same mini-game available if the player chooses to take it further.
